		...
		if (unit.className ~= "level") then		
			...
			elseif (#unit.colours == 0) then
				if (unit.values[A] > 0) and (math.floor(unit.values[A]) == unit.values[A]) then
					if (not is_parser(unit)) or (unit.active == false) then
					...
					for i,v in ipairs(targets) do
						...
						if (targetstill == nil) and floating(v,unitid,x,y) and (vunit.flags[DEAD] == false) then
							...
							if (odata.tele == nil) then
								if ((vname ~= name) or ((metatext_fixquirks and
										(getname(vunit,"text") == "text" and getname(unit,"text") == "text" and checkiftextrule(name,"is","tele",unitid)))
										or (get_broaded_str(name) == get_broaded_str(vname)) and checkiftextrule(name,"is","tele",unitid,nil,get_broaded_str(name))
                                        or (getmetalevel(vname) == getmetalevel(name) and checkiftextrule(name, "is", "tele", unitid, nil, "meta" .. getmetalevel(name)))))
										and (v ~= unitid) then
									local teles = istele
									...
									if (#teles > 1) then
										...
										for a,b in ipairs(teles) do
											...
											if (b ~= unitid) and (telename == name
													...
													or ((get_broaded_str(telename) == get_broaded_str(name)) and checkiftextrule(name,"is","tele",unitid,true,get_broaded_str(name)))
